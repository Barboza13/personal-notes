...
<template>
  <aside class="flex flex-col justify-start items-center h-full bg-(--default-background) py-2 px-4">
    <section class="flex justify-around items-center h-[4.6rem] w-full border-b border-gray-500">
      ...
      <button class="bg-red-500 hover:bg-red-600 text-(--text-color) cursor-pointer rounded-md transition-colors duration-75 ease-in p-2" @click="isOpenLogoutModal = true">
    ...
    <div v-show="isOpenLogoutModal" class="fixed inset-0 bg-black opacity-30 z-[999]"></div>
    <ShowConfirmModal>
      <div v-show="isOpenLogoutModal" @close="isOpenLogoutModal = false"
        ...
        <section class="flex justify-around items-center w-full">
          <button class="bg-gray-500 hover:bg-gray-600 cursor-pointer rounded-md transition-colors duration-75 ease-in py-2 px-4" @click="isOpenLogoutModal = false">Cancelar</button>
          <button class="bg-red-500 hover:bg-red-600 cursor-pointer rounded-md transition-colors duration-75 ease-in py-2 px-4" @click="logout">Cerrar sesion</button>
        </section>
      </div>
    </ShowConfirmModal>
  </aside>
