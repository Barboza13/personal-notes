...
export default class NoteService {
  ...
  async getAllNotes(userId: number): Promise<Note[] | undefined> {
    try {
      ...
      return await db.select('SELECT * FROM notes WHERE user_id = $1 AND deleted_at IS NULL ORDER BY id DESC', [userId])
    } catch (error) {
  ...
  async createNote(note: Note): Promise<MessageData | undefined> {
    try {
      const db = await this.dbService.getDatabase()
      const result: QueryResult = await db.execute('INSERT INTO notes (user_id, title, content) VALUES ($1, $2, $3)', [
        note.user_id,
        note.title,
        note.content,
      ])
      ...
      if (!result) {
        return { error: true, content: '¡Ocurrio un error al guardar la nota, por favor intente de nuevo!' }
      }
  ...
  async updateNote(id: number, note: Note): Promise<MessageData | undefined> {
    try {
      ...
      if (!result) {
        return { error: true, content: '¡Ocurrio un error al actualizar la nota, por favor intente de nuevo!' }
      }
  ...
  async deleteNote(id: number, deletedAt: string): Promise<MessageData | undefined> {
    try {
      const db = await this.dbService.getDatabase()
      const result: QueryResult = await db.execute('UPDATE notes SET deleted_at = $1 WHERE id = $2', [deletedAt, id])

      if (!result) {
        return { error: true, content: '¡Ocurrio un error al eliminar la nota, por favor intente de nuevo!' }
      }
